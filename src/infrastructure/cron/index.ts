...
import { AcademicRecordEntity } from '@/domain/entities/academicRecord.entity';
import DegreeDataSourceImpl from '../datasources/degree.datasource.impl';
import { EnrollmentDatasourceImpl } from '../datasources/enrollment.datasource.impl';
import { InstitutionDatasourceImpl } from '../datasources/institution.datasource.impl';
import { academicSelectionToMoodle, askForStudentData, getStudentData, sendToMoodleFromInscription } from '../utils/processor.util';
import AcademicSelectionDataSourceImpl from '../datasources/academicSelection.datasource.impl';
import appConstants from '@/shared/Constants';
import { StudentToMoodleDto } from '@/domain/dtos/student.dto';
...
export const syncInscriptions = CronJob.from({
    ...
    onTick: async () => {
        ...
        try {
            const inscriptions = await new InscriptionDatasourceImpl().getNotProcessedAfterNow()
            for (const inscription of inscriptions) {
                const degrees = await new DegreeDataSourceImpl().getByInscriptionUuid(inscription.uuid)
                if (degrees.length === 0) {
                    console.log(`Inscription with uuid ${inscription.uuid} does not have degrees`)
                    continue
                }
                const academicRecord = new AcademicRecordEntity(
                    inscription,
                    await new EnrollmentDatasourceImpl().getByInscriptionUuid(inscription.uuid),
                    await new InstitutionDatasourceImpl().getByDegrees(degrees),
                    degrees
                )
                
                await sendToMoodleFromInscription(academicRecord)
            }
        } catch (error) {
